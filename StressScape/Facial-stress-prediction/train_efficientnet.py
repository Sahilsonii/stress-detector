...
def build_model():
    ...
    model = Model(inputs=base_model.input, outputs=outputs, name='EfficientNetB0_Stress')
...
RESULTS_DIR = BASE_DIR / "results" / "EfficientNetB0"
IMG_SIZE = (224, 224)
BATCH_SIZE = 8
EPOCHS = 30
FINE_TUNE_EPOCHS = 10
...
def train_efficientnet():
    ...
    logger = SystemLogger('EfficientNetB0_training', str(RESULTS_DIR / 'logs'))
    ...
    try:
        ...
        with open(RESULTS_DIR / 'classification_report.txt', 'w') as f:
            f.write(f"EfficientNetB0 Classification Report\n{'='*60}\n\n{report}\n\n")
            f.write("Confusion Matrix:\n")
            f.write(str(confusion_matrix(y_true, y_pred)))
        
        # Generate reports
        logger.info("Generating reports...")
        generate_individual_report('EfficientNetB0', history1, y_true, y_pred, y_pred_probs, metrics)
