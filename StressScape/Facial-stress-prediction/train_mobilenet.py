...
def build_model():
    ...
    model = Model(inputs=base_model.input, outputs=outputs, name='MobileNetV2_Stress')
...
def generate_individual_report(model_name, history, y_true, y_pred, y_pred_probs, metrics):
    ...
    • Model: {model_name}<br/>
    • Total Epochs: {metrics['total_epochs']}<br/>
...
RESULTS_DIR = BASE_DIR / "results" / "MobileNetV2"
IMG_SIZE = (224, 224)
BATCH_SIZE = 8
EPOCHS = 30
FINE_TUNE_EPOCHS = 10
...
def train_mobilenet():
    ...
    logger = SystemLogger('MobileNetV2_training', str(RESULTS_DIR / 'logs'))
    ...
    try:
        ...
        callbacks = [
            ...
            tf.keras.callbacks.EarlyStopping(
                monitor='val_loss', patience=10, restore_best_weights=True, mode='min', verbose=1
            ),
            tf.keras.callbacks.ReduceLROnPlateau(
                monitor='val_loss', factor=0.3, patience=5, min_lr=1e-7, mode='min', verbose=1
            )
        ]
        ...
        with open(RESULTS_DIR / 'classification_report.txt', 'w') as f:
            f.write(f"MobileNetV2 Classification Report\n{'='*60}\n\n{report}\n\n")
            f.write("Confusion Matrix:\n")
            f.write(str(confusion_matrix(y_true, y_pred)))
        
        logger.info("Generating reports...")
        generate_individual_report('MobileNetV2', history1, y_true, y_pred, y_pred_probs, metrics)
